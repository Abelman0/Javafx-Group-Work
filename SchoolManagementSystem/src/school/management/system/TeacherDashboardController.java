...
public class TeacherDashboardController implements Initializable {
    ...
    public void addMarkList() {
        ...
        if (studentID_tf.getText().isEmpty() || studentName_tf.getText().isEmpty()
            ...
            try {
                ...
                String checkCourseID = "SELECT courseID FROM course WHERE department = '" + dept + "' AND courseID ='" + courseID_tf.getText() + "'";
    ...
    public void updateMarkList() {
        String sql = "UPDATE grade SET quizGrade = ?, testGrade = ?, assignmentGrade = ?, midGrade = ?, finalGrade = ?, totalGrade = ?, grade = ? "
                + "WHERE studentID = '" + studentID_tf.getText()
                + "' AND courseID = '" + courseID_tf.getText()
                + "' AND teacherID = '" + Data.userID + "'";
        ...
        if (studentID_tf.getText().isEmpty() || studentName_tf.getText().isEmpty()
            ...
            try {
                ...
                if (result.next()) {
                    ...
                    prepare = connect.prepareStatement(sql);
                    prepare.setString(1, quiz_tf.getText());
                    prepare.setString(2, test_tf.getText());
                    prepare.setString(3, project_tf.getText());
                    prepare.setString(4, mid__tf.getText());
                    prepare.setString(5, final_tf.getText());
    ...
    @FXML
    public void deleteMarkList() throws SQLException {

        String sql = "DELETE FROM grade WHERE studentID = '" + studentID_tf.getText() + "' AND courseID = '" + courseID_tf.getText() + "'";

        connect = DatabaseConnection.connectionDB();
        if (studentID_tf.getText().isEmpty() || courseID_tf.getText().isEmpty()) {

            errorMessage("Please fill StudentID and courseID fields");                                                                 // if one of them is null or empty it calles the emptyFields method
            ...
            alert.setHeaderText(null);
            alert.setContentText("Are you sure you want to DELETE Student ID: " + studentID_tf.getText() + "?");
    ...
    @FXML
    public void markListSelect() {
        GradeData gD = markList_tableView.getSelectionModel().getSelectedItem();   // It first gets the GradeData object(specific instance) corresponding to the selected row usign getSelectedItem() method
        int num = markList_tableView.getSelectionModel().getSelectedIndex();         // this also gets the index of the selected row   

        if ((num - 1) < -1) {       // if there no is selected row i.e index < 0  it simply return wihtout doing anything
            return;
        }

        studentID_tf.setText(gD.getStudentID());
        studentName_tf.setText(gD.getName());
        courseID_tf.setText(gD.getCourseID());
        quiz_tf.setText(String.valueOf(gD.getQuizGrade()));
        test_tf.setText(String.valueOf(gD.getTestGrade()));
        project_tf.setText(String.valueOf(gD.getAssignmentGrade()));
        mid__tf.setText(String.valueOf(gD.getMidGrade()));
        final_tf.setText(String.valueOf(gD.getFinalGrade()));
    }
    ...
    public void logout() {
        try {
            ...
            if (option.isPresent() && option.get().equals(ButtonType.OK)) {    // it used to check weather user clike the button(Ok OR Cancel) or not(X) close the dialog box 
                SchoolManagementSystem.sceneFactory("LoginFxml");
